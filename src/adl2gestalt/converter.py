...
class MedmToGestaltConverter:
    ...
    def convert_display(self, medm: MedmMainWidget) -> str:
        ...
        if hasattr(medm, 'geometry') and medm.geometry:
            lines.append(f"    geometry: {medm.geometry.width}x{medm.geometry.height} x {medm.geometry.x}x{medm.geometry.y}")
    ...
    def convert_widget_to_lines(self, widget: Any, index: int, color_table: List) -> List[str]:
        """
        ...
        if hasattr(widget, 'title') and widget.title:
            ...
            safe_title = widget.title[:20].replace(' ', '_').replace('/', '_').replace(':', '')
        ...
        if hasattr(widget, 'geometry') and widget.geometry:
            ...
            lines.append(f"    geometry: {geom.width}x{geom.height} x {geom.x}x{geom.y}")
    ...
    def add_widget_properties_lines(self, widget: Any, lines: List[str], widget_type: str, color_table: List) -> None:
        """
        ...
        if "control" in contents:
            ...
            if isinstance(control, dict):
                ...
                if "clr" in control:
                    fg_color = self.get_color_reference(int(control["clr"]), color_table)
                    if fg_color:
                        lines.append(f"    foreground: {fg_color}")
                if "bclr" in control:
                    bg_color = self.get_color_reference(int(control["bclr"]), color_table)
                    if bg_color:
                        lines.append(f"    background: {bg_color}")
            ...
            if isinstance(monitor, dict):
                if "chan" in monitor:
                    ...
                    if pv:
                        lines.append(f'    pv: "{pv}"')
                if "clr" in monitor:
                    fg_color = self.get_color_reference(int(monitor["clr"]), color_table)
                    if fg_color:
                        lines.append(f"    foreground: {fg_color}")
                if "bclr" in monitor:
                    bg_color = self.get_color_reference(int(monitor["bclr"]), color_table)
                    if bg_color:
                        lines.append(f"    background: {bg_color}")
        ...
        if widget_type in ["ProgressBar", "Gauge", "Slider", "Scale"]:
            if "direction" in contents:
                orientation = "vertical" if contents["direction"] in ["up", "down"] else "horizontal"
                lines.append(f'    horizontal: {str(orientation == "horizontal").lower()}')
            ...
            if widget.displays:
                ...
                for display in widget.displays:
                    if "name" in display:
                        ...
                        lines.append(f'        - {{ label: "{label}", file: "{display["name"]}", macros: "{macros}" }}')
