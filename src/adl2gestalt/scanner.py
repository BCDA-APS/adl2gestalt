...
def get_gestalt_path_for_medm(medm_file: Path, medm_folder: Path, gestalt_folder: Path) -> Path:
    """
...
def get_conversion_status(medm_file: Path, gestalt_folder: Path) -> Dict[str, Any]:
    ...
    Dict
        ...
        - 'status': Status string ('converted', 'outdated', 'pending')
    """
    medm_file = Path(medm_file)
    gestalt_folder = Path(gestalt_folder)
    
    # Find expected gestalt file path
    # Assume same relative structure with .yml extension
    ...
    if gestalt_file.exists():
        ...
        if status['medm_modified'] and status['gestalt_modified']:
            ...
            status['status'] = 'converted' if status['up_to_date'] else 'outdated'
    
    return status


def identify_pending_conversions(
    medm_folder: Path, 
    gestalt_folder: Path,
    recursive: bool = True
) -> List[Tuple[Path, str]]:
    """
    Return list of (medm_file, status) for files needing conversion.
    ...
    -------
    List[Tuple[Path, str]]
        List of tuples with MEDM file path and status:
        - 'pending': Never converted
        - 'outdated': Converted but MEDM file is newer
    """
    medm_folder = Path(medm_folder)
    gestalt_folder = Path(gestalt_folder)
    ...
    for medm_file in medm_files:
        expected_gestalt = get_gestalt_path_for_medm(medm_file, medm_folder, gestalt_folder)
        status = get_conversion_status(medm_file, expected_gestalt.parent)
        
        if status['status'] in ['pending', 'outdated']:
            pending.append((medm_file, status['status']))
...
def get_conversion_summary(
    ...
    for medm_file in medm_files:
        ...
        if status['status'] == 'converted':
            ...
            summary['pending'].append(medm_file)
    
    summary['total_converted'] = len(summary['converted']) + len(summary['up_to_date'])
    summary['total_outdated'] = len(summary['outdated'])
    summary['total_pending'] = len(summary['pending'])
